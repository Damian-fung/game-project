...
- ⚔️ **雙 AI 對戰**：兩條蛇同時訓練，互相學習對抗策略
- 🎮 **2D/3D 視覺化**：支援傳統 2D 與等角投影 3D 顯示模式
- 📊 **訓練監控**：即時顯示學習曲線、探索率、勝率等統計數據
- 💾 **自動保存**：模型與訓練數據自動持久化
- 🔍 **不同探索策略**：Snake 1 使用 ε-greedy，Snake 2 使用 Boltzmann 探索
...
⚠️ **必須使用 Python 3.10 或 3.11**（不支援 3.12+）

- [Python 3.11 下載](https://www.python.org/downloads/release/python-3119/)
- [Python 3.10 下載](https://www.python.org/downloads/release/python-31011/)
- 安裝時請勾選 **"Add Python to PATH"**

驗證安裝：

```bash
python --version  # 必須顯示 3.10.x 或 3.11.x
```
...
│   ├── snake1_model.pth    # AI 蛇 1 的神經網路權重
│   └── snake2_model.pth    # AI 蛇 2 的神經網路權重
│
...
**Snake 1（ε-greedy，快速探索）**：

- 學習率：0.001
- 探索率：1.0 → 0.01（衰減率 0.995）
- 折扣因子 γ：0.95
- 批次大小：32
- 記憶容量：10,000

**Snake 2（Boltzmann，溫度探索）**：

- 學習率：0.0008
- 探索率：0.8 → 0.05（衰減率 0.998）
...
2. **驗證版本**：
   ...
   ```bash
   python --version  # 必須是 3.10.x 或 3.11.x
   ```
