...
- **Boltzmann探索**：使用溫度參數的 Softmax 動作選擇
  - 根據Q值計算每個動作的概率：$P(a) = \frac{e^{Q(a)/T}}{\sum e^{Q(a')/T}}$
  - 溫度參數 T：1.0 → 0.1（逐漸降低）
  - 高溫度：接近隨機探索（均勻分佈）
  - 低溫度：偏向高Q值動作（貪心策略）
  - 優勢：平滑的探索-利用過渡，避免ε-greedy的突變
...
2. **單方死亡（分出勝負）**：
   - AI蛇1死亡且AI蛇2存活 → **AI蛇2獲勝**
   - AI蛇2死亡且AI蛇1存活 → **AI蛇1獲勝**
   - 結果：勝利方累積1勝，記錄到訓練日誌
...
**新增功能**：
- ⚡ **加速模式**：按F鍵或點擊按鈕切換3倍速，加快訓練和對戰速度
- 🎨 **UI面板優化**：重新設計控制面板布局，增加元素間間距
- 🔘 **按鈕狀態顯示**：加速/3D/訓練模式按鈕顯示不同顏色狀態
- 📦 **動態高度計算**：控制面板高度根據按鈕數量自動調整

**性能改進**：
- ⚡ 加速模式下遊戲速度從10 FPS提升至30 FPS
- 📊 食物獎勵從+10提升至+20，鼓勵AI更積極追求食物

**界面優化**：
- 📊 AI狀態面板間距從10px增至15px
- 📊 中央信息面板縮減至80px，移除重複信息
- 📊 所有面板間間距統一增加至15px，防止重疊
- 📊 底部快捷鍵說明改為緊湊格式
